...
public class AuthenticationManager {
    ...
    public static synchronized String getDeviceID(Context context) {
        if (uniqueID == null) {
            uniqueID = Utils.INSTANCE.getInternalSettingString(context, Const.PREF_UNIQUE_ID, "");
            if (uniqueID.isEmpty()) {
                uniqueID = UUID.randomUUID()
                               .toString();
                Utils.INSTANCE.setInternalSetting(context, Const.PREF_UNIQUE_ID, uniqueID);
            }
        }
        return uniqueID;
    }
    ...
    private String getPrivateKeyString() throws NoPrivateKey {
        String key = Utils.INSTANCE.getInternalSettingString(mContext, Const.PRIVATE_KEY, "");
        if (key.isEmpty()) {
    ...
    public String getPublicKeyString() throws NoPublicKey {
        String key = Utils.INSTANCE.getInternalSettingString(mContext, Const.PUBLIC_KEY, "");
        if (key.isEmpty()) {
    ...
    private void uploadKey(String publicKey, final ChatMember member) {
        ...
        if (Utils.INSTANCE.getInternalSettingBool(mContext, Const.PUBLIC_KEY_UPLOADED, false)) {
                         ...
                         .deviceRegister(dr, new Callback<TUMCabeStatus>() {
                             ...
                             public void onResponse(Call<TUMCabeStatus> call, Response<TUMCabeStatus> response) {
                                 ...
                                 if (response.isSuccessful() && "ok".equals(response.body()
                                                                                    .getStatus())) {
                                     if (member != null) {
                                         Utils.INSTANCE.setInternalSetting(mContext, Const.PUBLIC_KEY_UPLOADED, true);
                                     }
                             ...
                             public void onFailure(Call<TUMCabeStatus> call, Throwable t) {
                                 Utils.INSTANCE.log(t, "Failure uploading public key");
                                 Utils.INSTANCE.setInternalSetting(mContext, Const.PUBLIC_KEY_UPLOADED, false);
                             }
                         });
    ...
    public void uploadPublicKey() throws NoPublicKey {
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                ...
                requestSavePublicKey.setParameter("pToken", Utils.INSTANCE.getSetting(AuthenticationManager.this.mContext, Const.ACCESS_TOKEN, ""));
    ...
    private void tryToUploadGcmToken() {
        ...
        if (Utils.INSTANCE.getInternalSettingBool(mContext, Const.PUBLIC_KEY_UPLOADED, false) && GoogleApiAvailability.getInstance()
    ...
    private void saveKeys(String privateKeyString, String publicKeyString) {
        Utils.INSTANCE.setInternalSetting(mContext, Const.PRIVATE_KEY, privateKeyString);
        Utils.INSTANCE.setInternalSetting(mContext, Const.PRIVATE_KEY_ACTIVE, false); //We need to remember this state in order to activate it later
        Utils.INSTANCE.setInternalSetting(mContext, Const.PUBLIC_KEY, publicKeyString);
    }
