...
public class GcmIdentificationService extends FirebaseInstanceIdService {
    ...
    public String register() throws IOException {
        ...
        Utils.INSTANCE.setInternalSetting(mContext, Const.GCM_INSTANCE_ID, iid.getId());
        Utils.INSTANCE.setInternalSetting(mContext, Const.GCM_TOKEN_ID, token);
    ...
    public void onTokenRefresh() {
        String refreshedToken = FirebaseInstanceId.getInstance()
                                                  .getToken();
        Utils.INSTANCE.setInternalSetting(this, Const.GCM_TOKEN_ID, refreshedToken);
    }

    public String getCurrentToken() {
        return Utils.INSTANCE.getInternalSettingString(this.mContext, Const.GCM_TOKEN_ID, "");
    }
    ...
    public void checkSetup() {
        ...
        if (token.isEmpty()) {
            ...
            if (!Utils.INSTANCE.getInternalSettingBool(mContext, Const.GCM_REG_ID_SENT_TO_SERVER, false)) {
    ...
    private void registerInBackground() {
        try {
            ...
            Utils.INSTANCE.setInternalSetting(mContext, Const.GCM_REG_ID_SENT_TO_SERVER, false);
            Utils.INSTANCE.setInternalSetting(mContext, Const.GCM_REG_ID_LAST_TRANSMISSION, new Date().getTime());

            // Let the server know of our new registration id
            GcmIdentificationService.this.sendTokenToBackend(token);

            Utils.INSTANCE.log("GCM registration successful");
        } catch (IOException ex) {
            Utils.INSTANCE.log("Error :" + ex.getMessage());
        }
    }
    ...
    private void sendTokenToBackend(String token) {
        ...
        if (token == null || token.isEmpty()) {
            Utils.INSTANCE.logv("Parameter missing for sending reg id");
            return;
        }
                ...
                .deviceUploadGcmToken(dgcm, new Callback<TUMCabeStatus>() {
                    ...
                    public void onResponse(@NonNull Call<TUMCabeStatus> call, @NonNull Response<TUMCabeStatus> response) {
                        ...
                        if (response.isSuccessful() && s != null) {
                            Utils.INSTANCE.logv("Success uploading GCM registration id: " + s.getStatus());

                            // Store in shared preferences the information that the GCM registration id
                            // was sent to the TCA server successfully
                            Utils.INSTANCE.setInternalSetting(mContext, Const.GCM_REG_ID_SENT_TO_SERVER, true);
                        } else {
                            Utils.INSTANCE.logv("Uploading GCM registration failed...");
                        }
                    }

                    @Override
                    public void onFailure(@NonNull Call<TUMCabeStatus> call, @NonNull Throwable t) {
                        Utils.INSTANCE.log(t, "Failure uploading GCM registration id");
                        Utils.INSTANCE.setInternalSetting(mContext, Const.GCM_REG_ID_SENT_TO_SERVER, false);
                    }
                });
    }
    ...
    private void checkRegisterIdUpdate(String regId) {
        ...
        long lastTransmission = Utils.INSTANCE.getInternalSettingLong(mContext, Const.GCM_REG_ID_LAST_TRANSMISSION, 0);
        Date now = new Date();
